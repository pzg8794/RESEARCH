# UW Information Science PhD — Autumn 2027

Last verified: 2026-09-09

Status: **OPEN — P0 HIGH FIT — NOT SUBMITTED**

Portal state: **Applicant account and Autumn 2027 Information Science PhD draft created in the existing g.rit.edu Chrome profile. Program selection is saved; application questions are open.**

- Official application window: September 1–December 2, 2026, 11:59 PM Pacific.
- Internal ready-for-review target: November 19, 2026.
- Portal: [UW Graduate School application](https://apply.grad.uw.edu/)
- Official requirements: [Application materials](https://ischool.uw.edu/programs/phd/admissions/application-materials)
- Official timeline: [Application process](https://ischool.uw.edu/programs/phd/admissions/application-process)
- Existing program brief: [programs/uw-information-science.md](../../programs/uw-information-science.md)
- Common evidence: [COMMON_DOSSIER.md](../../COMMON_DOSSIER.md)

## Working research identity

The smallest coherent agenda is **reliable and equitable AI decision support when evidence is incomplete, delayed, or uneven across people and institutions**. Health and scientific-information systems provide the primary high-stakes setting. Accessibility is an evaluation requirement, not an add-on. Teaching and computing access belong primarily in the personal/diversity contribution unless a specific research question needs them.

This preserves a computational core—machine learning, uncertainty-aware decision policies, reproducible evaluation, and disparity measurement—while explaining why institutional practice and human context affect technical validity. That is the reason UW Information Science may be a stronger home than a generic CS framing.

## Resume point

Start with a 30–60 minute faculty-evidence session in [FACULTY-FIT.md](FACULTY-FIT.md). Do not write polished statements until Piter confirms the research agenda and provisional faculty order.

## Workspace

- [Requirements](REQUIREMENTS.md)
- [Portal fields](PORTAL-FIELDS.md)
- [Faculty fit](FACULTY-FIT.md)
- [Research statement plan](RESEARCH-STATEMENT-PLAN.md)
- [Personal statement plan](PERSONAL-STATEMENT-PLAN.md)
- [Diversity statement plan](DIVERSITY-STATEMENT-PLAN.md)
- [Materials checklist](MATERIALS-CHECKLIST.md)
- [Open questions and short sessions](OPEN-QUESTIONS.md)

## Evidence state

The public requirements and deadline are verified, and the secure applicant portal was inspected in Piter's authenticated account. The correct program and Autumn 2027 term are saved. No recommendation invitation, payment, or submission occurred. Subjective choices and sensitive fields remain deliberately unfilled rather than guessed.
